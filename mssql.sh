...
if ! docker ps -a --format '{{.Names}}' | grep -w devmssql &> /dev/null; then
   ...
   docker run --name devmssql -e 'ACCEPT_EULA=Y' -e 'MSSQL_SA_PASSWORD=Password1!' -p 1433:1433 -v /docker/mssql:/var/opt/mssql/data -v /docker/mssql/log:/var/opt/mssql/log -v /docker/mssql/secrets:/var/opt/mssql/secrets -d mcr.microsoft.com/mssql/server:2019-latest
else
